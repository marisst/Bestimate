...
def get_issue_count(repository_search_url, auth=None, jql=""):
    ...
    repository_search_url -- JIRA repository REST API search endpoint URL,
    e.g. 'https://jira.exoplatform.org/rest/api/latest/search'

    auth -- authentication parameters including username and API key or password
    can optionally be passed to gain access to more issues

    jql -- a JQL query narrowing down the issue pool, such as 'timespent > 0'
    to return the count of all issues with time spent greater than zero
    """
    ...
    if response.status_code != 200:
        print("%s returned unexpected status code %d when trying to get number of issues with the following JQL query: %s"
...
def test_repos(potential_jira_repo_url_list, min_labeled_issue_count):
    """Test a list of URLs and return JIRA repository URLs with publicly available 'timespent' field
    and at least min_labeled_issue_count labeled issues.  A labeled issue is a resolved issue
    with 'timespent' reported, which is greater than zero.

    Arguments:

    potential_jira_repo_url_list -- a list containing potential JIRA repository URLs,
    such as ['https://jira.go2group.com/secure/Dashboard.jspa', 'not-jira-url.com', 'issues.apache.org/jira']
...
def print_test_result(result, min_labeled_issue_count):
    ...
    if result["too_small"] > 0:
        print("%d were disqualified before they contained less than %d labeled issues."
            % (result["too_small"], min_labeled_issue_count))
    ...
    for repo in result["open_repos"]:
        print("%s - %d labeled issues, %d total issues, %.2f%% labeling coverage"
            % (repo["url"], repo["labeled_issues"], repo["total_issues"], repo["labeling_coverage"]))
