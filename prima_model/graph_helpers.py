...
def plot_losses(axs, training_losses, testing_losses, mean_baseline, median_baseline, divide_by = 1):
    ...
    min_value = min([min(training_losses), min(testing_losses), mean_baseline, median_baseline]) / divide_by #min(training_losses) #
    max_value = max([max(training_losses), max(testing_losses), mean_baseline, median_baseline]) / divide_by #max(training_losses) #
    loss_range = max_value - min_value

    axs.clear()
    axs.set_title("Training and testing losses") #axs.set_title("Predict next word in JIRA issue text") #
    axs.set_ylabel("Mean absolute error, hours", fontsize=FONTSIZE) #axs.set_ylabel("Accuracy", fontsize=FONTSIZE) #
